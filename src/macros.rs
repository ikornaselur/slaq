...
macro_rules! video {
    ...
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; title = $value:expr) => {
        $title = Some($crate::blocks::PlainText::new($value));
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; video_url = $value:expr) => {
        $video_url = Some($value.into());
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; thumbnail_url = $value:expr) => {
        $thumb = Some($value.into());
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; alt_text = $value:expr) => {
        $alt = Some($value.into());
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; description = $value:expr) => {
        $desc = Some($crate::blocks::PlainText::new($value));
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; title_url = $value:expr) => {
        $title_url = Some($value.into());
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; provider_icon_url = $value:expr) => {
        $icon = Some($value.into());
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; provider_name = $value:expr) => {
        $provider_name = Some($value.into());
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; author_name = $value:expr) => {
        $author = Some($value.into());
    };
    (@assign $title:ident, $video_url:ident, $thumb:ident, $alt:ident, $desc:ident, $title_url:ident, $icon:ident, $provider_name:ident, $author:ident, $block_id:ident; block_id = $value:expr) => {
        $block_id = Some($value.into());
    };
...
macro_rules! image {
    ...
    (@assign $alt:ident, $img_url:ident, $sf_id:ident, $sf_url:ident, $title:ident, $block_id:ident; alt_text = $value:expr) => {
        $alt = Some($value.into());
    };
    (@assign $alt:ident, $img_url:ident, $sf_id:ident, $sf_url:ident, $title:ident, $block_id:ident; image_url = $value:expr) => {
        $img_url = Some($value.into());
    };
    (@assign $alt:ident, $img_url:ident, $sf_id:ident, $sf_url:ident, $title:ident, $block_id:ident; slack_file_id = $value:expr) => {
        $sf_id = Some($value.into());
    };
    (@assign $alt:ident, $img_url:ident, $sf_id:ident, $sf_url:ident, $title:ident, $block_id:ident; slack_file_url = $value:expr) => {
        $sf_url = Some($value.into());
    };
    (@assign $alt:ident, $img_url:ident, $sf_id:ident, $sf_url:ident, $title:ident, $block_id:ident; title = $value:expr) => {
        $title = Some($value.into());
    };
    (@assign $alt:ident, $img_url:ident, $sf_id:ident, $sf_url:ident, $title:ident, $block_id:ident; block_id = $value:expr) => {
        $block_id = Some($value.into());
    };
    (@assign $alt:ident, $img_url:ident, $sf_id:ident, $sf_url:ident, $title:ident, $block_id:ident; $unexpected:ident = $_value:expr) => {
...
macro_rules! file {
    ...
    (@assign $ext:ident, $block_id:ident; external_id = $value:expr) => {
        $ext = Some($value.into());
    };
    (@assign $ext:ident, $block_id:ident; block_id = $value:expr) => {
        $block_id = Some($value.into());
    };
    (@assign $ext:ident, $block_id:ident; $unexpected:ident = $_value:expr) => {
...
macro_rules! context {
    ...
    (@list $v:ident; ) => {};
    (@list $v:ident; mrkdwn($text:expr) , $($rest:tt)* ) => {{
        $v.push($crate::blocks::ContextElement::mrkdwn($text));
        context!(@list $v; $($rest)*);
    }};
    (@list $v:ident; mrkdwn($text:expr) ) => {{
        $v.push($crate::blocks::ContextElement::mrkdwn($text));
    }};
    (@list $v:ident; plain($text:expr) , $($rest:tt)* ) => {{
        $v.push($crate::blocks::ContextElement::plain_text($text));
        context!(@list $v; $($rest)*);
    }};
    (@list $v:ident; plain($text:expr) ) => {{
        $v.push($crate::blocks::ContextElement::plain_text($text));
    }};
    (@list $v:ident; image($url:expr, $alt:expr) , $($rest:tt)* ) => {{
        $v.push($crate::blocks::ContextElement::image($url, $alt));
        context!(@list $v; $($rest)*);
    }};
    (@list $v:ident; image($url:expr, $alt:expr) ) => {{
        $v.push($crate::blocks::ContextElement::image($url, $alt));
    }};
    // Convenience: allow string literals; default to plain text for consistency
    (@list $v:ident; $text:literal , $($rest:tt)* ) => {{
        $v.push($crate::blocks::ContextElement::plain_text($text));
        context!(@list $v; $($rest)*);
    }};
    (@list $v:ident; $text:literal ) => {{
        $v.push($crate::blocks::ContextElement::plain_text($text));
    }};
    // Convenience: explicit text($expr) treated as plain text
    (@list $v:ident; text($text:expr) , $($rest:tt)* ) => {{
        $v.push($crate::blocks::ContextElement::plain_text($text));
        context!(@list $v; $($rest)*);
    }};
    (@list $v:ident; text($text:expr) ) => {{
        $v.push($crate::blocks::ContextElement::plain_text($text));
    }};
